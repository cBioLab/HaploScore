...
int main(){
  ...
  for(int p = 1; p < 10; p++){
    ...
    for(int i = 1; i <= numofTest; i++){
      ...
      system(("./path -H " + haploSetfile1 + " -h " + haplofile + " -s " + haploScorefile1 + " -p " + to_string((double)0.1*p)).c_str());
      system(("./path -H " + haploSetfile2 + " -h " + haplofile + " -s " + haploScorefile2 + " -p " + to_string((double)0.1*p)).c_str());
